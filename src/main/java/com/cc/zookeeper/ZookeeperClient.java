...
public class ZookeeperClient extends Logging {
		...
		for (AsyncRequest request : requests) {
			try {
				...
				send(request, new CallBack() {
					@Override
					public void execute(AsyncResponse response) {
	...
	private <E extends AsyncRequest> void send(E request, CallBack callBack) {
			...
			case EXISTS:
				...
				zooKeeper.exists(existsRequest.path(), shouldWatch(existsRequest), new AsyncCallback.StatCallback() {
					...
					public void processResult(int rc, String path, Object ctx, Stat stat) {
						callBack.execute(new ExistsResponse(KeeperException.Code.get(rc), path, ctx, stat, existsRequest));
					}
				}, request.ctx());
				break;
			case GETDATA:
				...
				zooKeeper.getData(getDataRequest.path(), shouldWatch(getDataRequest), new AsyncCallback.DataCallback() {
					...
					public void processResult(int rc, String path, Object ctx, byte[] data, Stat stat) {
						callBack.execute(new GetDataResponse(KeeperException.Code.get(rc), path, ctx, data, stat, getDataRequest));
					}
				}, request.ctx());
				break;
			case GETCHILDREN:
				...
				zooKeeper.getChildren(getChildrenRequest.path(), shouldWatch(getChildrenRequest), new AsyncCallback.Children2Callback() {
					...
					public void processResult(int rc, String path, Object ctx, List<String> children, Stat stat) {
						callBack.execute(new GetChildrenResponse(KeeperException.Code.get(rc), path, ctx, children, stat, getChildrenRequest));
					}
				}, request.ctx());
				break;
			case CREATE:
				...
				zooKeeper.create(request.path(), createRequest.data(), createRequest.acl(), createRequest.createMode(), new AsyncCallback.StringCallback() {
					...
					public void processResult(int rc, String path, Object ctx, String name) {
						callBack.execute(new CreateResponse(KeeperException.Code.get(rc), path, ctx, name, createRequest));
					}
				}, request.ctx());
				break;
			case SETDATA:
				...
				zooKeeper.setData(setDataRequest.path(), setDataRequest.data(), setDataRequest.version(), new AsyncCallback.StatCallback() {
					@Override
					public void processResult(int rc, String path, Object ctx, Stat stat) {
						callBack.execute(new SetDataResponse(KeeperException.Code.get(rc), path, ctx, stat, setDataRequest));
					}
				}, setDataRequest.ctx());
				break;
			case DELETE:
				...
				zooKeeper.delete(deleteRequest.path(), deleteRequest.version(), new AsyncCallback.VoidCallback() {
					...
					public void processResult(int rc, String path, Object ctx) {
						callBack.execute(new DeleteResponse(KeeperException.Code.get(rc), path, ctx, deleteRequest));
					}
				}, deleteRequest.ctx());
				break;
			case GETACL:
				...
				zooKeeper.getACL(getAclRequest.path(), null, new AsyncCallback.ACLCallback() {
					...
					public void processResult(int rc, String path, Object ctx, List<ACL> acl, Stat stat) {
						callBack.execute(new GetAclResponse(KeeperException.Code.get(rc), path, ctx, acl, stat, getAclRequest));
					}
				}, request.ctx());
				break;
			case SETACL:
				...
				zooKeeper.setACL(setACLRequest.path(), setACLRequest.acl(), setACLRequest.version(), new AsyncCallback.StatCallback() {
					@Override
					public void processResult(int rc, String path, Object ctx, Stat stat) {
						callBack.execute(new SetAclResponse(KeeperException.Code.get(rc), path, ctx, stat, setACLRequest));
					}
				}, setACLRequest.ctx());
				break;
		}
	...
	private interface CallBack {
		void execute(AsyncResponse response);
	}
